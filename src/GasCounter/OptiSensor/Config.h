...
namespace PinConfigurations
{
  ...
  // MODE (OUTPUT) -- NOT USED by this sensor. The Optomax sensor has no
  // control pin of its own, unlike the DFRobot SEN0368 capacitive sensor
  // (see capacitive_sensor_variant/Config.h), which needs one shared
  // GPIO driven HIGH once at boot before its OUT pins mean anything.
  //
  // -1 is a SENTINEL VALUE meaning "no MODE pin wired" -- GasChannel::
  // begin() (see GasChannel.cpp) checks for this before touching the
  // pin. This constant exists here, always defined to -1, specifically
  // so that GasChannel.cpp itself never needs to change when swapping
  // between this sensor and the capacitive one -- only Config.h does.
  // The capacitive variant's Config.h defines this same constant, but
  // set to a real GPIO number instead of -1.
  const int LEVEL_SENSOR_MODE_PIN = -1;
  //
